...
void GeometryHelper::CreateGrid(shared_ptr<Geometry<VertexTextureData>> geometry, int32 sizeX, int32 sizeY)
{
	...
	for (int32 y = 0; y < sizeY + 1; y++)
	{
		...
		{
			VertexTextureData v;
			v.position = Vec3(static_cast<float>(x), static_cast<float>(y), 0);
			v.uv = Vec2(static_cast<float>(x), static_cast<float>(y));
	...
	for (int32 y = 0; y < sizeY; y++)
	{
		for (int32 x = 0; x < sizeX; x++)
		{
			...
			//   |	\
			//  [2] - [1]
			idx.push_back((sizeX + 1) * (y + 1) + (x));
			idx.push_back((sizeX + 1) * (y)+(x + 1));
			idx.push_back((sizeX + 1) * (y)+(x));
			//  [1] - [2]
			//   	\  |
			//		  [0]
			idx.push_back((sizeX + 1) * (y)+(x + 1));
			idx.push_back((sizeX + 1) * (y + 1) + (x));
			idx.push_back((sizeX + 1) * (y + 1) + (x + 1));
		}
	}
